(function day2() {
    ...
    const parseDay2Values = (values) => {
        ...
        games.forEach((game) => {
            ...
            parsedGame.Id = parseInt(game.substring(5, colonIndex));

            let cubeGrabs = game.substring(colonIndex + 1).split(';');

            let grabsParsed = [];

            cubeGrabs.forEach((cubeGrab) => {
                let cubes = cubeGrab.split(',');
                let grabParsed = {};

                cubes.forEach((cube) => {
                    cube = cube.trim();

                    let value = parseInt(cube.substring(0, cube.indexOf(' ')));
                    let color = cube.substring(cube.indexOf(' ') + 1);

                    grabParsed[color] = value;
                });

                grabsParsed.push(grabParsed);
            });

            parsedGame.grabs = grabsParsed;
    ...
    const solveDay2 = () => {
        ...
        games.forEach((game) => {
            ...
            for (let g in game.grabs) {
                ...
                if (grab.red > totalRedCubes || grab.green > totalGreenCubes || grab.blue > totalBlueCubes) {
                    isValid = false;
                    break;
                }
            }
            ...
            if (isValid === false) {
                ...
                return; //proceed to the next loop
            }
    ...
    const day2values = `Game 1: 1 blue, 2 green, 3 red; 7 red, 8 green; 1 green, 2 red, 1 blue; 2 green, 3 red, 1 blue; 8 green, 1 blue
