(function day3() {
    ...
    const solveDay3 = () => {
        ...
        for (let s = 0; s < schematicLines.length; s++) {
            ...
            for (let l = 0; l < schematicLine.length; l++) {
                ...
                // if the next char is not a number, reset the part number
                if (l === schematicLine.length - 1 || isNaN(schematicLine[l + 1])) {
                    partNumber = null;
                }
            }
        }
    ...
    const getPartNumber = (line, startingIndex) => {
        let partNumber = "";
        
        for (let p = startingIndex; p < line.length; p++) {
            let currentChar = line[p];
    ...
    const checkCurrentColumnIsSpecial = (schematicLines, s, l) => {
        ...
        if (s < schematicLines.length - 1 && isSpecialChar(schematicLines[s + 1][l])) {
            return true;
        }
